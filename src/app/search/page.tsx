...
import { getZaraCategory, zaraCategories } from "@/data/zaraParser";
  ...
  return (
    ...
    >
      <Link href={`/product/${product.id}`} className={styles.cardImgWrap}>
        {renderMedia(hovered && img2 ? img2 : img1)}
        {/* Availability badge */}
...
function SearchContent() {
  ...
  // Get all products
  const allProducts = zaraCategories.flatMap((cat) => getZaraCategory(cat.key));
  ...
  const queryFiltered = allProducts.filter((p) => 
    ...
    (p.familyName && p.familyName.toLowerCase().includes(q.toLowerCase()))
  );

  // Unique families for filter tabs based on query filtered products
  const families = ["ALL", ...Array.from(new Set(queryFiltered.map((p) => p.familyName).filter(Boolean)))];
    ...
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader} ref={headerRef}>
        <div className={styles.breadcrumb}>
          <Link href="/">NIL</Link> / SEARCH RESULTS
        </div>
        <h1 className={styles.catTitle}>"{q}"</h1>
